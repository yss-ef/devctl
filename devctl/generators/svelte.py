...
def generate_svelte_boilerplate(project_name: str) -> bool:
    ...
    try:
        ...
        typer.secho(
            f"✅ Svelte project '{safe_name}' successfully generated!", fg=typer.colors.GREEN
        )
        return True

    except subprocess.CalledProcessError as e:
        typer.secho(f"❌ Svelte creation failed with code: {e.returncode}", fg=typer.colors.RED)
        return False
    except Exception as e:
        typer.secho(f"❌ Svelte initialization failed: {e}", fg=typer.colors.RED)
        return False
