...
def launch_dev_environment(env_state: dict):
    ...
    try:
        ...
        if env_state.get("has_vue"):
            typer.secho(
                f"🟢 Démarrage de Vue.js depuis {env_state['vue_path']}...",
        ...
        if env_state["has_docker_compose"]:
            ...
            typer.secho(
                f"🐳 Démarrage de Docker depuis {env_state['docker_path']}...",
        ...
        if env_state["has_angular"]:
            typer.secho(
                f"🅰️ Démarrage d'Angular depuis {env_state['angular_path']}...",
                fg=typer.colors.RED
            )
        ...
        typer.secho(
            "✅ Nettoyage terminé. Environnement parfaitement propre.",
            fg=typer.colors.GREEN
        )
        sys.exit(0)
