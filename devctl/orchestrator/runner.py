...
def launch_dev_environment(projects: List[DockerProject], docker_composes: List[Path]):
    ...
    try:
        ...
        for p in frontends:
            ...
            icon = "🅰️" if p.kind == "angular" else ("🟢" if p.kind == "vue" else "⚛️")
            cmd = ["npx", "ng", "serve"] if p.kind == "angular" else ["npm", "run", "dev"]
            
            typer.secho(f"{icon} Starting {p.kind.capitalize()}: {p.name}...", fg=typer.colors.CYAN if p.kind == "angular" else (typer.colors.MAGENTA if p.kind == "vue" else typer.colors.BLUE))
